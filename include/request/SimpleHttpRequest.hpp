...
void _LOGI() {}
template <typename T, typename... Args> void _LOGI(T t, Args &&...args) {
  if (getenv("DEBUG_LOG")) {
    try {
...
template <typename T, typename... Args> void LOGI(T t, Args &&...args) {
  if (!getenv("DEBUG"))
    return;
...
static uv_alloc_cb uvAllocCb = [](uv_handle_t *handle, size_t size,
                                  uv_buf_t *buf) {
...
class SimpleHttpRequest {
public:
  SimpleHttpRequest(const map<string, string> &options, uv_loop_t *loop)
      : SimpleHttpRequest(loop) {
    this->options = options;
  }
  SimpleHttpRequest(const map<string, string> &options,
                    const map<string, string> &requestHeaders)
      : SimpleHttpRequest(uv_default_loop()) {
    this->options = options;
    this->requestHeaders = requestHeaders;
  }
  SimpleHttpRequest(const map<string, string> &options,
                    const map<string, string> &requestHeaders, uv_loop_t *loop)
      : SimpleHttpRequest(loop) {
    this->options = options;
    this->requestHeaders = requestHeaders;
  }
  ...
  SimpleHttpRequest(uv_loop_t *loop) : uv_loop(loop) {
    ...
    parser_settings.on_message_begin = [](http_parser *parser) { return 0; };
    parser_settings.on_url = [](http_parser *p, const char *buf, size_t len) {
      LOGI("Url: ", string(buf, len));
      return 0;
    };

    parser_settings.on_header_field = [](http_parser *p, const char *buf,
                                         size_t len) {
    ...
    parser_settings.on_header_value = [](http_parser *p, const char *buf,
                                         size_t len) {
      ...
      string field =
          string(client->lastHeaderFieldBuf, client->lastHeaderFieldLenth);
    ...
    parser_settings.on_headers_complete = [](http_parser *parser) {
      ...
      LOGI("on_headers_complete. status code : ",
           std::to_string(parser->status_code));
    ...
    parser_settings.on_body = [](http_parser *parser, const char *buf,
                                 size_t len) {
      SimpleHttpRequest *client = (SimpleHttpRequest *)parser->data;
      if (buf)
  ...
  SimpleHttpRequest &on(const string name,
  ...
  // GET/HEAD verb
  SimpleHttpRequest &get(const string &url) { return _get(url, "GET"); }
  SimpleHttpRequest &head(const string &url) { return _get(url, "HEAD"); }
  ...
  void end() {
    ...
    if (r != 0) {
      ...
      r = uv_getaddrinfo(uv_loop, &getaddrinfo_req,
                         NULL, // sync
                         options["hostname"].c_str(), options["port"].c_str(),
                         &hints);
      if (r != 0) {
        LOGE("uv_getaddrinfo");
      ...
      char ip[17]{'\0'};
      r = uv_ip4_name((struct sockaddr_in *)getaddrinfo_req.addrinfo->ai_addr,
                      ip, 16);
    ...
    if (options.count("protocol") &&
      ...
      r = uv_timer_start(
          ...
          [](uv_timer_t *timer) {
            ...
            BIO_set_conn_hostname(client->sbio,
                                  string(client->options["hostname"] + ":" +
                                         client->options["port"])
                                      .c_str());
            ...
            stringstream res;
            res << client->options["method"] << " " << client->options["path"]
                << (client->options["query"].length() == 0
                        ? ""
            ...
            if (client->requestBody.tellp() > 0) {
              res << "Content-Length: "
                  << std::to_string(client->requestBody.tellp()) << "\r\n";
              res << "\r\n";
            ...
            for (;;) {
              ...
              parsed = (ssize_t)http_parser_execute(
                  parser, &client->parser_settings, tmpbuf, nread);
              LOGI("parsed: ", parsed);
              if (parser->upgrade) {
    ...
    r = uv_tcp_connect(
        &connect_req, &tcp, reinterpret_cast<const sockaddr *>(&dest),
          ...
          int r = uv_read_start(
              ...
              [](uv_stream_t *tcp, ssize_t nread, const uv_buf_t *buf) {
                ...
                if (nread > 0) {
                  http_parser *parser = &client->parser;
                  parsed = (ssize_t)http_parser_execute(
                      parser, &client->parser_settings, buf->base, nread);

                  LOGI("parsed: ", parsed);
                  if (parser->upgrade) {
                    LOGE("raise upgrade unsupported");
                    client->emit("error", "upgrade",
                  ...
                  if (nread != UV_EOF) {
                    LOGE("uv_read_cb");
                    client->emit("error", uv_err_name(nread),
                                 uv_strerror(nread));
                    return;
                  }
                }
          ...
          r = uv_write(&client->write_req, req->handle, &resbuf, 1,
                       [](uv_write_t *req, int status) {
                         SimpleHttpRequest *client =
                             (SimpleHttpRequest *)req->data;
                         LOGI("uv_write");
                         if (status != 0) {
                           LOGE("uv_write_cb");
                           client->emit("error", uv_err_name(status),
                                        uv_strerror(status));
                           return;
                         }
                       });
...
private:
  ...
  SimpleHttpRequest &_post(const string &url, const string &method,
                           const string &body) {
    options["method"] = method;
    if (!_parseUrl(url)) {
  ...
  bool _parseUrl(const string &url) {
    ...
    options["protocol"] =
        url.substr(u.field_data[UF_SCHEMA].off, u.field_data[UF_SCHEMA].len);
    options["protocol"] += ':';
    options["hostname"] =
        url.substr(u.field_data[UF_HOST].off, u.field_data[UF_HOST].len);
    options["port"] =
  ...
  void _clearConnection() {
    ...
    if (uv_is_active((uv_handle_t *)&this->tcp) &&
        !uv_is_closing((uv_handle_t *)&this->tcp)) {
